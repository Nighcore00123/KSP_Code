...
def Negative_Roll (Roll_Difference, Roll_Rate_Of_Change):
    ...
    elif (0.0005 <= Roll_Difference and -0.001 < Roll_Rate_Of_Change < -0.0005):
        Soothing_rate = 0.01
        Exponential_Rate = 2
        Roll_Command = Soothing_rate * (abs(Roll_Difference) ** Exponential_Rate)
        Roll_Command = max(-1, min(1, Roll_Command))  # Ensure pitch command is between -1 and 1
...
def Positive_Roll(Roll_Difference, Roll_Rate_Of_Change):
    ...
    if (Roll_Difference < 0.0000 and Roll_Rate_Of_Change < 0.0005):
        Soothing_rate = 0.0075
        Exponential_Rate = 2
        Roll_Command = Soothing_rate * (abs(Roll_Difference) ** Exponential_Rate)
        Roll_Command = max(-1, min(1, Roll_Command))  # Ensure pitch command is between -1 and 1
    ...
    elif (-0.0010 < Roll_Difference < -0.0005 and 0.0005 < Roll_Rate_Of_Change <= 0.001):
        Soothing_rate = 0.01
        Exponential_Rate = 2
        Roll_Command = Soothing_rate * (abs(Roll_Difference) ** Exponential_Rate)
        Roll_Command = max(-1, min(1, Roll_Command))  # Ensure pitch command is between -1 and 1
